...
export default function VendorVerificationGuard({ children }) {
	...
	const handleRefresh = async () => {
		...
		if (updatedUser?.vendorVerificationStatus === "VERIFIED") {
			// Status updated, guard will automatically allow access
	...
	const state =
		user.vendorVerificationStatus === "REJECTED"
			? {
					...
					description:
						"Your vendor account has been reviewed and rejected. Please contact the admin or submit a new request if needed.",
					icon: "❌",
				}
			: {
					title: "Vendor approval pending",
					description:
						"Your vendor account has been created, but you must wait for admin verification before accessing the vendor portal.",
					icon: "⏳",
				};
		...
		<div className="min-h-screen bg-slate-50 p-6">
			<div className="mx-auto max-w-4xl space-y-6 pt-10">
				...
				<PageHero
					...
					description="Vendor access is restricted until admin verification is completed."
				/>
					...
					<div className="mt-4 space-y-3">
						...
						<div className="flex items-start gap-3">
							...
							<div>
								...
								<div className="text-sm text-slate-600">
									An administrator will review your vendor registration and approve it soon.
								</div>
							</div>
						</div>
						<div className="flex items-start gap-3">
					...
					<div className="flex items-start gap-3">
						...
						<div>
							...
							<p className="mt-1 text-sm text-indigo-700">
								If you have questions about your vendor account status, please contact the
								administrator. You'll receive a notification once your account is approved.
							</p>
						</div>
					</div>
				</div>
